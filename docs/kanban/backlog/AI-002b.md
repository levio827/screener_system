...
- [ ] Models registered with version, metrics, and artifacts
- [ ] Daily Airflow DAG runs successfully at 2 AM KST
- [ ] DAG includes model evaluation and automatic promotion
- [ ] ModelService loads production model on startup
- [ ] API endpoint `/v1/ai/predict/{stock_code}` returns valid predictions
- [ ] Predictions include confidence scores (0-100%)
- [ ] Batch prediction supports up to 100 stocks
- [ ] Redis caching reduces latency by >80% on cache hits
- [ ] Cached predictions refresh daily
- [ ] Model info endpoint returns current version and metrics
- [ ] 18+ tests passing (unit + integration)
- [ ] API response time <200ms p95 (with cache), <500ms p95 (without cache)
